...
#define SIMULATION_REPETITIONS 20
...
void tree_simulation(int n)
{
    ...
    ins_file.open("../csvdata/zad2rand/BSTCRInsertRandomData.csv");
    del_file.open("../csvdata/zad2rand/BSTCRDeleteRandomData.csv");
    ins_height_file.open("../csvdata/zad2rand/BSTHInsertRandomData.csv");
    del_height_file.open("../csvdata/zad2rand/BSTHDeleteRandomData.csv");
    ...
    for(int i = 1; i <= ITERATIONS_MULTIPLIER; i++)
    {
        ...
        for(k = 0; k < SIMULATION_REPETITIONS; k++)
        {
            ...
            {   
                int node_value = mersenne_generator(0, UPPER_LIMIT(tree_size));
